// THE CONTROL THAT MAKES THE PREVIOUS RESULT MEAN SOMETHING, OR KILLS IT.
//
...
console.log("rule   | FLOOR  ORDER-1  ORDER-2  CEILING | ORDER-1 as % of ceiling | headroom");
for (const rule of ['FORCED', 'CHOICE'] as const) {
  ...
  console.log(
    `${rule.padEnd(6)} | ${res.FLOOR.toFixed(2)}   ${res['ORDER-1'].toFixed(2)}     ` +
      `${res['ORDER-2'].toFixed(2)}     ${res.CEILING.toFixed(2)} | ${pct.toFixed(1).padStart(22)}% | ` +
      `${(100 - pct).toFixed(1)}%`,
  );
}
